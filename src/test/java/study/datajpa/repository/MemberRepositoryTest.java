...
class MemberRepositoryTest {
    ...
    public void returnType() {
        ...
        List<Member> result = memberRepository.findListByUsername("AAA"); // 조회 결과가 없을 경우 빈 List 리턴
        assertThat(result.get(0)).isEqualTo(m1);

        Member findMember = memberRepository.findMemberByUsername("AAA"); // 조회 결과가 없을 경우 null 리턴(단건)
        assertThat(findMember).isEqualTo(m1);

        Optional<Member> findOptionalMember = memberRepository.findOptionalByUsername("AAA");
        assertThat(findOptionalMember).isEqualTo(Optional.of(m1));
    }
...
//        em.clear();
    ...
    public void findMemberLazy() {
        ...
        //select Member
        // N + 1 문제
        // 문제 해결 위해 fetch join 사용
        // 혹은 EntityGraph 사용(Override)
...
//        Member findMember = memberRepository.findById(member1.getId()).get();
    ...
    @Test
    public void specBasic() { // 실무에서는 JPA Criteria를 거의 안 쓴다(복잡)! 대신에 QueryDSL을 사용하자.
        //given
        Team teamA = new Team("teamA");
        em.persist(teamA);
    ...
    public void queryByExample() {
        ...
        //when
        //Probe
        // 도메인 객체를 사용하여 검색 조건으로 활용
        // QueryByExample은 inner 조인만 가능하여 실무에서 실용성이 떨어진다.
...
//        List<UsernameOnly> result = memberRepository.findProjectionsByUsername("m1");
    ...
    @Test
    public void nativeQuery() { // JPA에서는 Native Query는 지양하는 것이 좋고, 최후의 수단으로 사용하는 것이 좋다
        //given
        Team teamA = new Team("teamA");
        em.persist(teamA);
...
//        System.out.println("result = " + result);
        ...
        Page<MemberProjection> result = memberRepository.findByNativeProjection(PageRequest.of(0, 10)); // projections 활용
